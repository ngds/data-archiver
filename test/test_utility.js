...
module.exports = {
	...
	test_buildDirs: function (test){
		...
		test.ok(fs.existsSync(path.join(base, "outputs", "archive")), "directory outputs/archive not created correctly");
		test.ok(fs.existsSync(path.join(base, "outputs", "logs")), "directory outputs/logs not created correctly");
		test.done();
	},

	// Test utility.buildUrl
	test_buildUrl: function (test){
		test.expect(2);
