...
function constructor (dirs, item, option, callback) {

  function wfs (dirs, item) {
  ...
  if (option === 'wfs') {
    var data = wfs(dirs, item);
    if (data.length > 0) callback(null, data);
  }

  if (option === 'csw') {
    var data = csw(dirs, item);
    if (data.length > 0) callback(null, data);
  }

  if (option === 'all') {
    var cswData = csw(dirs, item);
    var wfsData = wfs(dirs, item);
    if (wfsData.length > 0) {
      for (var i = 0; i < wfsData.length; i++) {
        cswData.push(wfsData[i]);
      }
    }
    callback(null, cswData);
  }
}
