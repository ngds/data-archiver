      ...
      parse.parseCsw(get, function (data) {
        if (data) {
          var counter = data.length
            , increment = 0;
          function recursiveProcess (item) {
            sync.constructor(dirs, item, host, function (linkages) {
              if (linkages) {
                sync.processor(linkages, function () {
                  increment++;
                  if (increment < counter) {
                    recursiveProcess(data[increment]);
                  }
                  if (increment === counter && data['next']) {
                    var start = Number(data['next']);
                    console.log(start);
                    if (start > 0 && start <= max)
                      recursiveScrape(csw, start, increment, max);
